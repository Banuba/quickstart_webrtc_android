...
class Geometry {
    constructor(filename, name) {
        ...
        let mesh = assets$4.findMesh(name);
        if (!mesh) {
            mesh = assets$4.createMesh(name);
            assets$4.uploadMeshData(mesh, filename);
        }
        this.$$ = mesh;
    }
}
...
class BeautyMorphing {
    constructor() {
        ...
        if (!mesh) {
            mesh = assets$4.createMesh(name);
            assets$4.uploadMeshData(mesh, "$builtin$meshes/beauty");
        }
        this.$$ = mesh;
    }
    weight(type, value) {
...
class LUT {
    constructor(filename, onLoad) {
        ...
        const lut = (_a = assets$1.findImage(name)) !== null && _a !== void 0 ? _a : assets$1.createImage(name, bnb.ImageType.LUT);
        this.$$ = lut;
        if (filename)
...
class Image {
    constructor(filename, onLoad) {
        ...
        const image = assets$1.findImage(name) || assets$1.createImage(name, bnb.ImageType.TEXTURE);
        this.$$ = image;
        if (filename)
            this.load(filename, onLoad);
    }

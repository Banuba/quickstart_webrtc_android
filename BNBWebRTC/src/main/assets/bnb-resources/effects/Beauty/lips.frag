...
void main()
{
...
#else
	vec3 bg = texture(glfx_BACKGROUND, var_uv.xy).rgb;
#endif
	...
	float alpha = (texture( glfx_LIPS_MASK, var_uv.zw )[int(glfx_LIPS_MASK_T[0].w)]);

	F = vec4(blendColor(bg, js_lips_color.xyz, alpha * js_lips_color.w, js_lips_brightness_contrast.xy), 1.);
}

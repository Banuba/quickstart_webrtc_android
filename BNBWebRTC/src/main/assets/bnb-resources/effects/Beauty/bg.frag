...
layout(std140) uniform glfx_GLOBAL
{
  ...
  highp vec4 js_skinsoftening_removebags_rotation;
    ...
    // isMirrored flag
	highp vec4 js_is_screen_Mirrored;
...
vec2 scale_to_fill_uv (vec2 uv, float angle, sampler2D sampler){
    vec2 textureSize = vec2(textureSize(sampler, 0));
    float aspect_ratio_y = glfx_SCREEN.y / glfx_SCREEN.x;
    float aspect_ratio_x = glfx_SCREEN.x / glfx_SCREEN.y;
...
vec2 aspect_fit_uv (vec2 uv, float angle, sampler2D sampler){
    vec2 textureSize = vec2(textureSize(sampler, 0));
    float aspect_ratio_y = glfx_SCREEN.y / glfx_SCREEN.x;
    float aspect_ratio_x = glfx_SCREEN.x / glfx_SCREEN.y;
    float texture_aspect_ratio = textureSize.y / textureSize.x;
    ...
    if (texture_aspect_ratio > aspect_ratio) {
        scale_y = 1./texture_aspect_ratio * aspect_ratio;
...
vec2 aspect_fill_uv_tex(vec2 uv, float angle, float scale)
{
    vec2 texSize = vec2(textureSize(bgTex, 0));
    ...
    float inv_scale_x = 1. / scale_x / scale;
    float inv_scale_y = 1. / scale_y / scale;
...
{   
    ...
    vec4 bg = texture(glfx_BACKGROUND,uv);
    uv.y = 1. - uv.y;
    ...
    float angle = 0.;
    if( side.y > 0.7071 && glfx_SCREEN.x < glfx_SCREEN.y)
    	angle = 90. * js_is_screen_Mirrored.x;
    else if( side.y < -0.7071 && glfx_SCREEN.x < glfx_SCREEN.y)
    	angle = -90. * js_is_screen_Mirrored.x;
    else if( up.y < -0.7071 && glfx_SCREEN.x < glfx_SCREEN.y)
    	angle = 180.;
    
    if (js_bg_rotation.y == 1.) {
        angle = js_bg_rotation.x;
    } else {
        angle += js_bg_rotation.x;
    }

    float scale = js_bg_scale.x;
    
    if (angle != 0.) uv = rotate_uv(uv,angle);
	uv = aspect_fill_uv_tex(uv,angle,scale);

    if(js_bg_content_mode.x == 2.){
        uv = aspect_fit_uv(uv,angle,bgTex);
    } else if (js_bg_content_mode.x == 3.){
        uv = scale_to_fill_uv(uv,angle,bgTex);
    }

    vec4 bg_color = texture(bgTex, uv);
    
    const float threshold = 0.2;

    float mask = max((textureBicubic(glfx_BG_MASK,var_bg_mask_uv).x - threshold) / (1.0 - threshold), 0.0);

    F = vec4(bg_color.rgb, mask*bg_color.a);
}
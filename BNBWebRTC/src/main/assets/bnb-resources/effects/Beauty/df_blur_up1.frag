...
void main()
{
	vec3 sum = 
		(1./6.)*(
			...
			texture(tex_df_blur_u1,var_uv.zw + vec2(-js_bg_blur_radius.x, 0)/vec2(textureSize(tex_df_blur_u1,0))).xyz + 
			texture(tex_df_blur_u1,var_uv.zw + vec2( 0,-js_bg_blur_radius.x)/vec2(textureSize(tex_df_blur_u1,0))).xyz + 
			texture(tex_df_blur_u1,var_uv.zw + vec2(-js_bg_blur_radius.x,-js_bg_blur_radius.x)/vec2(textureSize(tex_df_blur_u1,0))).xyz) + 
		(1./12.)*(
			texture(tex_df_blur_u1,var_uv.xy + vec2( js_bg_blur_radius.x, 0)/vec2(textureSize(tex_df_blur_u1,0))).xyz + 
			texture(tex_df_blur_u1,var_uv.xy + vec2(-js_bg_blur_radius.x, 0)/vec2(textureSize(tex_df_blur_u1,0))).xyz + 
			texture(tex_df_blur_u1,var_uv.xy + vec2( 0, js_bg_blur_radius.x)/vec2(textureSize(tex_df_blur_u1,0))).xyz + 
			texture(tex_df_blur_u1,var_uv.xy + vec2( 0,-js_bg_blur_radius.x)/vec2(textureSize(tex_df_blur_u1,0))).xyz);
